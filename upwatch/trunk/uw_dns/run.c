...
void refresh_database(MYSQL *mysql)
{
  ...
  sprintf(qry,  "SELECT pr_dns_def.id, pr_dns_def.domid, pr_dns_def.tblid, pr_domain.name, "
                "       pr_dns_def.ipaddress, "
                "       pr_dns_def.yellow,  pr_dns_def.red "
                "FROM   pr_dns_def, pr_domain "
                "WHERE  pr_dns_def.id > 1 and pr_dns_def.disable <> 'yes'"
                "       and pr_dns_def.pgroup = '%d' and pr_domain.id = pr_dns_def.domid",
...
void write_probe(gpointer key, gpointer value, gpointer user_data)
{
  ...
  if (probe->domain) {
    xmlSetProp(dns, "domain", probe->domain);
  }
