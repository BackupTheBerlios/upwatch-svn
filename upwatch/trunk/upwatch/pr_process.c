...
void *get_def(trx *t, int create)
{
  ...
  if (def == NULL) {
    ...
    result = db_query(t->probe->db, 0, "select * from pr_%s_def where id = '%u'", 
    ...
    if (dbi_result_get_numrows(result) == 0) { // DEF RECORD NOT FOUND
      ...
      result = db_query(t->probe->db, 0,
                        "insert into pr_%s_def (server, ipaddress, description) "
                        "            values ('%d', '%s', '%s')",
      ...
      res->probeid = dbi_conn_sequence_last(t->probe->db, sequence);
      ...
      result = db_query(t->probe->db, 0, "select ipaddress, description, server, yellow, red, "
...
void *get_def_by_servid(trx *t, int create)
{
  ...
  if (dbi_result_get_numrows(result) == 0) { // DEF RECORD NOT FOUND
    ...
    dbi_result_free(result);
    result = db_query(t->probe->db, 0,
                      "insert into pr_%s_def (server, ipaddress, description) values ('%u', '%s', '%s')", 
                       res->name, res->server, res->ipaddress ? res->ipaddress : "", 
                       res->hostname);
    sprintf(sequence, "pr_%s_def_id_seq", res->name);
